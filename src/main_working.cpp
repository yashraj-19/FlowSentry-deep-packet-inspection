...
int main(int argc, char* argv[]) {
    ...
    // Export stats.json
    {
        ...
        for (size_t i = 0; i < sorted_apps.size(); i++) {
            const auto& [app, count] = sorted_apps[i];
            json << "    \"" << appTypeToString(app) << "\": " << count;
            if (i + 1 < sorted_apps.size()) json << ",";
            json << "\n";
        }
        json << "  }\n";
        json << "}\n";
    }
    std::cout << "[Stats] Exported to stats.json\n";

    // Top 5 SNI domains by packet count
    std::vector<std::pair<std::string, uint64_t>> sni_counts;
    {
        std::unordered_map<std::string, uint64_t> sni_map;
        for (const auto& [tuple, flow] : flows)
            if (!flow.sni.empty()) sni_map[flow.sni] += flow.packets;
        sni_counts.assign(sni_map.begin(), sni_map.end());
    }
    std::sort(sni_counts.begin(), sni_counts.end(),
              [](const auto& a, const auto& b) { return a.second > b.second; });
    if (sni_counts.size() > 5) sni_counts.resize(5);

    std::cout << "\n+----------------------------------+---------+\n";
    std::cout << "| Domain                           | Packets |\n";
    std::cout << "+----------------------------------+---------+\n";
    for (const auto& [sni, count] : sni_counts) {
        std::string d = sni.size() > 32 ? sni.substr(0, 32) : sni;
