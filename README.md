# FlowSentry — Deep Packet Inspection Engine

A multi-threaded DPI engine in C++ for parsing network captures, extracting TLS SNI, classifying applications, and filtering flows based on rules.

## Attribution
Forked and extended from [perryvegehan/Packet_analyzer](https://github.com/perryvegehan/Packet_analyzer). Original architecture credit goes to the upstream author.
...
- TLS Client Hello parsing and SNI extraction from unencrypted handshake bytes
- Producer-consumer architecture with thread-safe queues, mutexes, and condition variables
- Five-tuple flow tracking and rule-based filtering at the flow level
